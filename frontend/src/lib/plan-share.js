...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR SAATH — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { SAATH_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.SAATH_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t a SAATH plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with SAATH'))}</footer>
</div></body></html>`
}
