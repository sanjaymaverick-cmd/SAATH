...
If you run SAATH for other people, you have had no way to answer "who signed in, and when?" —
the server kept no record of anything. It does now: an **activity log** in the admin dashboard,
covering sign-ins, sign-outs, the attempts that failed, and every admin action. This historical
entry otherwise remains a record of the inherited release and does not describe current hosting.
...
### Historical browser demo

- The inherited release included a browser-only demonstration build with seeded example data.
  SAATH does not currently claim a hosted public demo.
...
- **Nothing changes for using SAATH.** It never shipped that media — not in the repository,
...
- **It does change what you may do with the media.** Reusing the images or animations — in
  SAATH or anywhere else, commercially or not — needs your own licence from Gym visual. See
  [NOTICE.md](NOTICE.md).
...
- 📊 **<https://github.com/sanjaymaverick-cmd/SAATH> now runs self-hosted, cookieless
...
- 🔒 **Your instance does not.** The frontend only gets an analytics tag when
  ...
  image or in a plain `npm run build`. A self-hosted SAATH remains telemetry-free, as advertised.
...
- **A per-muscle exercise breakdown** behind the muscle card — estimated 1RM per exercise, decay
  ...
  [#92](https://github.com/sanjaymaverick-cmd/SAATH/pull/92),
  [#93](https://github.com/sanjaymaverick-cmd/SAATH/pull/93) and
  [#94](https://github.com/sanjaymaverick-cmd/SAATH/pull/94).
- **Fatigue is now intensity-weighted**, not volume alone: a set counts for more the closer it is
  ...
  [#55](https://github.com/sanjaymaverick-cmd/SAATH/pull/55).
...
- 🔗 **Supersets advance properly.** Completing a set moves to the next member of the group, the
  ...
  [#80](https://github.com/sanjaymaverick-cmd/SAATH/pull/80).
- ➖ **Remove an exercise from a running session**, with a superset-aware picker and a confirmation.
  ...
  [#83](https://github.com/sanjaymaverick-cmd/SAATH/pull/83).
- ⬅️ **The Android back button closes the open sheet** instead of leaving the screen or the app
  ...
  Contributed by [@Space-Hermes](https://github.com/Space-Hermes) in
  [#85](https://github.com/sanjaymaverick-cmd/SAATH/pull/85).
...
- 🐳 **The API service no longer has to be called `api`** ([#99]). The web image builds its nginx
  ...
  [@GAS85](https://github.com/GAS85) in [#100](https://github.com/sanjaymaverick-cmd/SAATH/pull/100).
...
- 🏷️ **Health checks and OCI image labels** on both images — source, licence, version, revision and
  ...
  [@GAS85](https://github.com/GAS85) in [#98](https://github.com/sanjaymaverick-cmd/SAATH/pull/98).
...
[#63]: https://github.com/sanjaymaverick-cmd/SAATH/issues/63
[#87]: https://github.com/sanjaymaverick-cmd/SAATH/issues/87
[#99]: https://github.com/sanjaymaverick-cmd/SAATH/issues/99
...
Contributed by [@Space-Hermes](https://github.com/Space-Hermes) in
[#50](https://github.com/sanjaymaverick-cmd/SAATH/pull/50).
...
Contributed by [@Space-Hermes](https://github.com/Space-Hermes) in
[#64](https://github.com/sanjaymaverick-cmd/SAATH/pull/64).
...
Contributed by [@Space-Hermes](https://github.com/Space-Hermes) in
[#67](https://github.com/sanjaymaverick-cmd/SAATH/pull/67).
...
- 📥 **Imports mapped more of what other apps export** ([#74]). Treadmill, Goblet Squat, Cycling and
  ...
  overlap could reach the names SAATH stores them under. Those and their neighbours are now in
...
[#79]: https://github.com/sanjaymaverick-cmd/SAATH/issues/79
[#74]: https://github.com/sanjaymaverick-cmd/SAATH/issues/74
[#60]: https://github.com/sanjaymaverick-cmd/SAATH/pull/60
...
SAATH written by someone other than me. Thank you.
...
SAATH is also a standalone mobile app — and it ships as a direct APK download, not
through app stores.

- 📱 **Standalone mobile app.** The same frontend now also builds as a native iPhone /
  Android app (Capacitor) — the install-and-done flavor of SAATH: no account, no server,
  ...
  - Exercise images/animations load from the configured upstream CDN.
...
- 🤖 **Android APK, no Play Store.** The official build is a signed, sideloadable APK
  (~4.5 MB) from [github.com/sanjaymaverick-cmd/SAATH](https://github.com/sanjaymaverick-cmd/SAATH) — deliberately
...
A muscle map across the app, plus a historical browser-only demonstration build.
...
- ▶️ **Historical browser demo** — a browser-only build (`VITE_DEMO=1`) was published by the
  inherited project. It boots
...
- 🐳 The inherited project published multi-architecture Docker images through GitHub Actions.
  This is a historical fact, not a promise that current SAATH images are publicly available.
