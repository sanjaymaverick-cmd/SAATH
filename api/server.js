/* SAATH API — password auth + per-user state storage
...
const RP_NAME = process.env.RP_NAME || 'SAATH';
...
const routes = {
  ...
  'POST /api/push/test': async (req, res) => {
    const user = readSession(req);
    if (!user) return json(res, 401, { error: 'not signed in' });
    await sendPush(user.id, { title: 'SAATH', body: 'Test notification ✅ — this is what alerts look like.', tag: 'test' });
    json(res, 200, { ok: true });
  },
...
}).listen(PORT, () => console.log(`saath-api on :${PORT} (rpID=${RP_ID}, origin=${ORIGIN})`));
